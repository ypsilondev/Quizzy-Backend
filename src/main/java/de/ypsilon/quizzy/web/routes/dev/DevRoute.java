...
public class DevRoute implements Route {
    ...
    private static final String VERIFY_ACCOUNT_FORM = "<h1>Verify User:</h1> Note: you need to have a session cookie!<form target=\"_blank\" method=\"post\" onsubmit=\"submitForm(this); return false;\" action=\"/users/security/verify\"> <input type=\"text\" name=\"verificationNumber\" placeholder=\"Verification-Number\"> <input type=\"submit\" value=\"Verifiy!\"></form> <br />";
    ...
    private static final String SERIALISATION_FORM = "<script>function submitForm(e){let t=serializeForm(e),n=e.action,i=e.method;var l=new XMLHttpRequest;l.open(i,n),l.setRequestHeader(\"Content-Type\",\"application/json;charset=UTF-8\"),l.addEventListener(\"load\",function(e){document.getElementById(\"responseDisplay\").value=l.responseText}),l.send(t),document.getElementById(\"requestDisplay\").value=t}function serializeForm(e){let t={};for(child of e.children)\"INPUT\"==child.tagName&&\"submit\"!=child.type&&(t[child.name]=child.value);let n=JSON.stringify(t);return console.log(n),n}</script>";
